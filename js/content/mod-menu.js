// =========================================================================
// /js/content/mod-menu.js - CIEE Hub PRO V2.0 (Com Sincronização Inteligente)
// =========================================================================
...
window.injectKairosModMenu = function() {
    ...
    chrome.storage.sync.get({
        ...
        if (isCiee) {
            ...
            // --- NOVO BOTÃO: INJEÇÃO INTELIGENTE (MEMÓRIA DO HUB) ---
            bodyContainer.appendChild(createBtn('🚀 Injetar Acompanhamento', 'blue', () => {
                if (typeof window.executarInjecaoPortal !== 'function') {
                    return window.showCieeToast("Robô do Portal não encontrado. Dê F5 na página.", "error");
                }

                chrome.storage.local.get(['ciee_talentos_sync'], function(result) {
                    const syncData = result.ciee_talentos_sync;

                    if (syncData && syncData.log) {
                        let pacoteMenu = {
                            status: "Análise de CV/Recusou agendamento",
                            classificacao: "ANÁLISE DE CV",
                            dias_sla: syncData.isEnvio ? 3 : 2, 
                            categoria: "Acompanhamento",
                            ticket: syncData.ticket || "",
                            log: syncData.log
                        };
                        window.showCieeToast(`Sincronizado! (SLA de ${pacoteMenu.dias_sla} dias)`, "success");
                        window.executarInjecaoPortal(pacoteMenu);
                    } else {
                        // FALLBACK SILENCIOSO (Sem prompt para não bugar o Modal)
                        navigator.clipboard.readText().then(texto => {
                            if (!texto || texto.trim() === "") {
                                return window.showCieeToast("Nada encontrado no Hub ou na área de transferência.", "warning");
                            }
                            
                            let isEnvio = texto.toLowerCase().includes("envi");
                            
                            let pacoteMenu = {
                                status: "Análise de CV/Recusou agendamento",
                                classificacao: "ANÁLISE DE CV",
                                dias_sla: isEnvio ? 3 : 2, 
                                categoria: "Acompanhamento",
                                ticket: "", // Deixa vazio para o operador preencher na tela
                                log: texto
                            };
                            
                            window.showCieeToast(`Injetado sem Ticket (SLA: ${pacoteMenu.dias_sla} dias). Preencha o ticket na tela!`, "warning");
                            window.executarInjecaoPortal(pacoteMenu);
                            
                        }).catch(() => {
                            window.showCieeToast("Erro ao ler área de transferência e Hub vazio.", "error");
                        });
                    }
                });
            }));
...
// =========================================================================
// 🕵️ ESPIÃO DE TOKEN (Sincroniza o login do Portal com o Hub)
// =========================================================================
if (window.location.href.includes('talentos.cieerj.org.br')) {
    setInterval(() => {
        let sessionData = localStorage.getItem('sb-hcfsekhaqfosfjdlxmir-auth-token');
        if (sessionData) {
            chrome.storage.local.set({ 
                'ciee_talentos_token': JSON.parse(sessionData).access_token 
            });
        }
    }, 5000); // Salva o token renovado no cofre a cada 5 segundos
}