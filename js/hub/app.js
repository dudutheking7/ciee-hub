// ==========================================
// /js/hub/app.js - (V4.2 - Gerenciador de Lotes Inteligente + HTML Rich Text)
// ==========================================
...
async function copiarTextoEditavel(elementId, acao, btn) {
    ...
    try {
        ...
        if (typeof chrome !== 'undefined' && chrome.tabs && acao.includes("Zendesk")) {
            chrome.tabs.query({url: "*://*.zendesk.com/*"}, function(tabs) {
                tabs.forEach(tab => {
                    chrome.tabs.sendMessage(tab.id, {acao: "injetarZendesk", texto: text, ticket_alvo: ticket_global});
                });
            });
